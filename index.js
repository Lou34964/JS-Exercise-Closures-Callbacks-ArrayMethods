...
*/
function processLength(lst, cb) {
  return cb(lst.length)
}
...
*/
function processLastItem(lst, cb) {
  return cb(lst[lst.length -1]);
}
...
function processSum(num, cb) {
  ...
  return cb(num.reduce((a,b) => a+b, 0))
}
...
function getFullNames(runners) {
  return (runners.map((currentVallue) =>{
    return `${currentVallue.last_name}, ${currentVallue.first_name}`;
  }))
}
...
*/
function getRunnersByTShirtSize(runners, ts) {
  return runners.filter((curval)=>{
    return curval.shirt_size === ts;
  });
}
...
function tallyUpDonations(runners) {
  return runners.reduce((accumulator, curval) =>{
